...
interface PetSignOutButtonProps {
  caregiverEmail: string;
  ackToken: string;
}

export function PetSignOutButton({ caregiverEmail, ackToken }: PetSignOutButtonProps) {
  ...
  async function handleSignOut() {
    ...
    router.push(
      `/auth/login?email=${encodeURIComponent(caregiverEmail)}&redirect=${encodeURIComponent(`/acknowledge-pet?token=${ackToken}`)}`
    );
  }
